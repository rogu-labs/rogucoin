...
"""Class for rogucoind node under test"""
...
class TestNode():
    """A class for representing a rogucoind node under test.
    ...
    def __del__(self):
        # Ensure that we don't leave any rogucoind processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, stderr=None, *args, **kwargs):
        ...
        # unclean shutdown), it will get overwritten anyway by rogucoind, and
        # potentially interfere with our attempt to authenticate
        delete_cookie_file(self.datadir, self.chain)
        self.process = subprocess.Popen(all_args, stderr=stderr, *args, **kwargs)
        self.running = True
        self.log.debug("rogucoind started, waiting for RPC to come up")

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the rogucoind process. Returns False if unable to connect."""
        # Poll at a rate of four times per second
        poll_per_s = 4
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError(self._node_msg(
                    'rogucoind exited with status {} during initialization'.format(self.process.returncode)))
            try:
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcassword. rogucoind still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to rogucoind")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, partial_match=False, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to rogucoind
        expected_msg: regex that stderr should match when rogucoind fails

        Will throw if rogucoind starts without an error.
        Will throw if an expected_msg is provided and it does not match rogucoind's stdout."""
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                self.log.debug('rogucoind failed to start: %s', e)
                self.running = False
                self.process = None
                ...
                if expected_msg is None:
                    assert_msg = "rogucoind should have exited with an error"
                else:
                    assert_msg = "rogucoind should have exited with expected error " + expected_msg
...
class TestNodeCLI():
    """Interface to rogucoin-cli for an individual node"""
    ...
    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with rogucoin-cli command-line options
    ...
    def send_cli(self, command=None, *args, **kwargs):
        """Run rogucoin-cli command. Deserializes returned string as python object."""

        pos_args = [str(arg) for arg in args]
        named_args = [str(key) + "=" + str(value) for (key, value) in kwargs.items()]
        assert not (pos_args and named_args), "Cannot use positional arguments and named arguments in the same rogucoin-cli call"
        ...
        self.log.debug("Running rogucoin-cli command: %s" % command)
