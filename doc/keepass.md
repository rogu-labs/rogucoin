...
Every connection to KeePassHTTP server is done via plain HTTP and even though protocol uses some internal encryption it should not be considered to be a highly secure one. This protocol has certain flaw which allow an attacker to decrypt your passwords when they manage to intercept communication between a KeePassHTTP server over a network connection (see [here](https://github.com/pfn/keepasshttp/issues/258) and [here](https://github.com/keepassxreboot/keepassxc/issues/147)). Rogucoin Core therefore strictly limits communication between itself and KeePassHttp to your local computer. As long as your computer is not compromised, your wallet passphrase is as safe as if you would enter it directly.

### What's new

The following new options are available for rogucoind and rogucoin-qt:
...
The following rpc commands are available:
 ...
 - _keepass init_: sets up the association between rogucoind and keepass by generating an AES key and sending an association message to KeePassHttp. This will trigger KeePass to ask for an Id for the association. Returns the association and the base64 encoded string for the AES key.
...
Sample initialization flow from _rogucoin-qt_ console (this needs to be done only once to set up the association):

 - Have KeePass running with an open database
 - Start _rogucoin-qt_
 - Open console
 - Type "_keepass init_" in rogucoin-qt console
 - Keepass pops up and asks for an association id, fill that in, for example, "_mydrkwallet_"
 - You should get a response like this "_Association successful. Id: mydrkwalletrogucoin - Key: AgQkcs6cI7v9tlSYKjG/+s8wJrGALHl3jLosJpPLzUE=_"
 - Edit _rogucoin.conf_ and fill in these values
```
keepass=1
...
Extended guide with screenshots is also available: https://www.rogucoin.com/forum/threads/keepass-integration.3620/
