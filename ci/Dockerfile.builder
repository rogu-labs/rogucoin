...
# rogucoin_hash
RUN git clone https://github.com/rogu-labs/rogucoin_hash
RUN cd rogucoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} rogucoin
RUN useradd -u ${USER_ID} -g rogucoin -s /bin/bash -m -d /rogucoin rogucoin
...
RUN mkdir /rogucoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /rogucoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /rogucoin-src
