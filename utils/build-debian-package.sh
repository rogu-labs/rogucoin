...
PACKAGE_VERSION=$($SRC_PATH/src/rogucoind --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/rogucoind $DEB_BIN
cp $SRC_PATH/src/rogucoin-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/rogucoin-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/rogucoind.1 $DEB_MAN
cp $SRC_DOC/man/rogucoin-cli.1 $DEB_MAN
cp $SRC_DOC/man/rogucoin-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/rogucoind.bash-completion $DEB_CMP/rogucoind
cp $SRC_PATH/contrib/rogucoin-cli.bash-completion $DEB_CMP/rogucoin-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/rogucoind.1
gzip --best -n $DEB_MAN/rogucoin-cli.1
gzip --best -n $DEB_MAN/rogucoin-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/rogucoind $DEB_BIN/rogucoin-cli
