...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/RogucoinParams"
else
    PARAMS_DIR="$HOME/.rogucoin-params"
fi
...
DOWNLOAD_URL="https://github.com/rogu-labs/rogucoin/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with ROGU_DISABLE_SOMETHING=1
ROGU_DISABLE_WGET="${ROGU_DISABLE_WGET:-}"
#ROGU_DISABLE_IPFS="${ROGU_DISABLE_IPFS:-}"
ROGU_DISABLE_CURL="${ROGU_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$ROGU_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$ROGU_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$ROGU_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Rogucoin parameters!
...
main() {
    ...
    cat <<EOF
ROGU - fetch-params.sh

This script will fetch the Rogucoin SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Rogucoin SNARK parameters. Note that it is
