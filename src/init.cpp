...
public:
    ...
    // Rogucoin Specific WalletInitInterface InitCoinJoinSettings
 ...
 * between rogucoind, and rogucoin-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-sporkaddr=<rogucoinaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", false, OptionsCategory::DEBUG_TEST);
    ...
    gArgs.AddArg("-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by Rogucoin Platform, to the specified username.", false, OptionsCategory::SMARTNODE);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/rogu-labs/rogucoin>";
    const std::string URL_WEBSITE = "<https://rogucoin.com>";
...
/** Sanity checks
 *  Ensure that Rogucoin Core is running in a usable environment with all
...
{
    // Make sure only a single Rogucoin Core process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if Rogucoin Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Rogucoin Core is started while in a temporary directory.\n",
    ...
    // ********************************************************* Step 10c: schedule Rogucoin-specific tasks
