...
#if defined(NDEBUG)
# error "Rogucoin Core cannot be compiled without assertions."
#endif
                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in Rogucoin
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    //     exit(1);
    // }
    double nSubsidy = 0.001;      // (declaring the reward variable and its original/default amount)

    if (nPrevHeight == 0) {
        nSubsidy = 35000;
    } else if ((nPrevHeight <= 1) && (nPrevHeight < 720)) {
        nSubsidy = 0.001;
    } else if ((nPrevHeight >= 720) && (nPrevHeight < 21600)) {
        nSubsidy = 5;
    } else if ((nPrevHeight >= 21600) && (nPrevHeight < 43200)) {
        nSubsidy = 4.8;
    } else if ((nPrevHeight >= 43200) && (nPrevHeight < 64800)) {
        nSubsidy = 4.6;
    } else if ((nPrevHeight >= 64800) && (nPrevHeight < 259200)) {
        nSubsidy = 4.5;
    } else if ((nPrevHeight >= 259200) && (nPrevHeight < 518400)) {
        nSubsidy = 3.75;
    } else if ((nPrevHeight >= 518400) && (nPrevHeight < 777600)) {
        nSubsidy = 2.85;
    } else if ((nPrevHeight >= 777600) && (nPrevHeight < 1036800)) {
        nSubsidy = 1.73;
    } else if ((nPrevHeight >= 1036800) && (nPrevHeight < 1296000)) {
        nSubsidy = 0.21;
    } else if ((nPrevHeight >= 1296000) && (nPrevHeight < 1555200)) {
        nSubsidy = 0.14;
    } else if ((nPrevHeight >= 1555200) && (nPrevHeight < 1814400)) {
        nSubsidy = 0.12;
    } else if ((nPrevHeight >= 1814400) && (nPrevHeight < 2073600)) {
        nSubsidy = 0.08;
    } else if ((nPrevHeight >= 2073600) && (nPrevHeight < 2332800)) {
        nSubsidy = 0.03;
    } else if ((nPrevHeight >= 2332800) && (nPrevHeight < 2592000)) {
        nSubsidy = 0.01;
...
void ThreadScriptCheck() {
    RenameThread("rogucoin-scriptch");
...
static int64_t nTimeRogucoinSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    /// ROGUCOIN: Check superblock start
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, view, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(ROGUCOIN): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    // ROGUCOIN : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if (llmq::RejectConflictingBlocks()) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(ROGUCOIN): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else if (!fReindex && !fImporting) {
        LogPrintf("ConnectBlock(ROGUCOIN): spork is off, skipping transaction locking checks\n");
    }
    ...
    // ROGUCOIN : MODIFIED TO CHECK SMARTNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, (blockReward + specialTxFees), strError)) {
        return state.DoS(0, error("ConnectBlock(ROGUCOIN): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward, specialTxFees)) {
        return state.DoS(0, error("ConnectBlock(ROGUCOIN): couldn't find smartnode or superblock payments"),
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeRogucoinSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - Rogucoin specific: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4), nTimeRogucoinSpecific * MICRO, nTimeRogucoinSpecific * MILLI / nBlocksTotal);
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, inputs, false /*fJustCheck*/, false /*fScriptChecks*/)) {
        return error("RollforwardBlock(ROGU): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), FormatStateMessage(state));
    }
