...
const char * const BITCOIN_CONF_FILENAME = "rogucoin.conf";
const char * const BITCOIN_PID_FILENAME = "rogucoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "rogucoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RogucoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RogucoinCore
    // Mac: ~/Library/Application Support/RogucoinCore
    // Unix: ~/.rogucoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RogucoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RogucoinCore";
#else
    // Unix
    return pathRet / ".rogucoincore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty rogucoin.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Rogucoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Rogucoin Core developers";
    }
