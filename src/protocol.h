...
enum ServiceFlags : uint64_t {
    ...
    // set by all Rogucoin Core non pruned nodes, and is unset by SPV clients or other light clients.
    NODE_NETWORK = (1 << 0),
    // NODE_GETUTXO means the node is capable of responding to the getutxo protocol request.
    // Rogucoin Core does not support this but a patch set called Bitcoin XT does.
    // See BIP 64 for details on how this is implemented.
    NODE_GETUTXO = (1 << 1),
    // NODE_BLOOM means the node is capable and willing to handle bloom-filtered connections.
    // Rogucoin Core nodes used to support this by default, without advertising this bit,
...
enum GetDataMsg {
    ...
    MSG_SPORK = 6,
    /* 7 - 15 were used in old Rogucoin versions and were mainly budget and MN broadcast/ping related*/
    MSG_DSTX = 16,
